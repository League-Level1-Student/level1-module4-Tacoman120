...
public class Jeopardy implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == firstButton) {
			askQuestion("Pistacios are not actually nuts, they are ________?", "fruit", 200);
		} else if(e.getSource() == secondButton) {
			askQuestion("What was the first food eaten in space?", "Apple Sauce", 400);	
		} else if(e.getSource() == thirdButton) {
			askQuestion("What green vegetable contains more food than steak?", "Broccoli", 600);
		} else if(e.getSource() == fourthButton) {
			askQuestion("Cotton candy was created by a _______?", "Dentist", 800);
		} else if(e.getSource() == fifthButton){
			askQuestion("Cucumers are ____% water?", "95", 1000);
		}
	...
	private void askQuestion(String question, String correctAnswer, int prizeMoney) {
		...
		if(ask.equalsIgnoreCase(correctAnswer)) {
			score+=prizeMoney;
